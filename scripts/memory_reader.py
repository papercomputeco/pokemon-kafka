...
TYPE_ID_MAP: dict[int, str] = {
    ...
    0x14: "fire",
    0x15: "water",
    # pokered type constants: GRASS $16, ELECTRIC $17, PSYCHIC $18, ICE $19. Both pairs were
    # swapped here until 2026-08-26, so every stored enemy_type label from before then lies the
    # same way (the encounter catalog exposed it: 5,772 wild Pikachu rows typed "grass", and the
    # Mt. Moon Lass's Oddish/Bellsprout typed "electric"). scripts/encounters.py un-swaps legacy
    # streams by timestamp; the battle strategy's move scoring was mis-typed against these four
    # types for every fight before the fix.
    0x16: "grass",
    0x17: "electric",
    0x18: "psychic",
    0x19: "ice",
...
# Internal species id -> name, extracted from the ROM's own name table (10 bytes per entry,
# internal-id order, found by scanning for RHYDON = id 0x01) — 151 real entries, MISSINGNO
# slots skipped. Extracted, not recalled: the hand-typed map this replaces carried two lies
# (0x6D "Metapod" was PARAS — hiding 6,515 wild grass-type sightings inside Mt. Moon — and
# 0x6E "Kakuna" was POLIWHIRL); scripts/encounters.py un-swaps those labels in old streams.
