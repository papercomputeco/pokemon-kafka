...
def test_enforced_power_limit_parses_nvidia_smi_or_returns_none(monkeypatch):
    monkeypatch.setattr(lm.subprocess, "run", lambda *a, **k: _R("600.00\n"))
    assert lm.enforced_power_limit() == 600.0
    monkeypatch.setattr(lm.subprocess, "run", lambda *a, **k: _R(""))
    assert lm.enforced_power_limit() is None
    monkeypatch.setattr(lm.subprocess, "run", lambda *a, **k: (_ for _ in ()).throw(OSError("no nvidia-smi")))
    assert lm.enforced_power_limit() is None
...
def test_power_check_refuses_capped_models_at_stock_and_when_unreadable():
    ...
    ok, why = lm.power_check(dense, None)
    assert ok is False and "could not report" in why
    # a model without a note is fine at any limit, including an unreadable one
