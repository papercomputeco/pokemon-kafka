...
|---|---|---|---|
| route1-flee-loop | battle | route1-navigation-flee-loop | reaches Viridian Forest (51) from `route1.state` in ≤2000 turns with lead HP ≥5. In a clean worktree the lane never enters the flee loop (916 turns, 9/9 battles, both before and after the 08-16 wedge watchdog + stall-run cap) but arrives at 2 HP, so it still FAILs on the health half |
| forest-crossing-healthy | navigation · battle | viridian-forest-turn-385-blackout | from the 17-HP forest entrance, `very_cautious` reaches Pewter (2) in ≤3000 turns with lead HP ≥5. Before 08-16 this row was a `PYTHONHASHSEED` coin flip (PASS 2270 / hp 13 or FAIL 3000 stuck in the Forest): `_pick_move` broke physical/special damage ties by set order. Ties now fall back to the best-scored move, and the lane is reproducible — currently FAIL 1251 turns / Pewter / hp 2 (reaches the city, misses min_lead_hp) |
